#!/usr/bin/env python3
"""LightGBM transaction monitoring model training on AMLSim outputs."""
...
def train_model(df: pd.DataFrame, numeric_cols, categorical_cols, test_size, random_state, model_dir: Path, simulation_name: str):
    ...
    model_path = model_dir / f"{simulation_name}_lightgbm.joblib"
    metrics_path = model_dir / f"{simulation_name}_lightgbm_metrics.json"
...
def main():
    ...
    print(f"LightGBM Metrics saved to {metrics_path}")
    ...
    if baseline:
        ...
        for key in ["roc_auc", "average_precision"]:
            if key in baseline and key in metrics:
                ...
                comparison[key] = {
                    "lightgbm": metrics[key],
                    "baseline": baseline[key],
                    "diff": diff
                }
        print(json.dumps(comparison, indent=2))
    else:
        print("\nNo baseline metrics found for comparison. Run train_simple_model.py first.")
